...
def showHosters(sHtmlContent = '', sTitle = False):
    ...
    sPattern = '<li id="Hoster.*?rel="([^"]+)".*?<div class="Named">(.*?)</div>'
    oParser = cParser()
    aResult = oParser.parse(sHtmlContent, sPattern)
    if (aResult[0]):
        for aEntry in aResult[1]:
            ...
            if (oHoster != False):
                __createMenuEntry(oGui, 'getHosterUrlandPlay', sHoster, [['siteUrl', sUrl], ['sTitle', sTitle],
                  ['securityCookie', sSecurityValue]])
    oGui.setEndOfDirectory()
    ...
    if (len(aMovieParts) > 1):
        ...
        for sPartUrl in aMovieParts:
          ...
          if (oHoster != False):
                ...
                #oOutputParameterHandler.addParameter('Name', oHoster.get_host())
                oOutputParameterHandler.addParameter('sTitle', sTitle+" Part "+str(iCounter))
                oOutputParameterHandler.addParameter('Hoster', oHoster)
                oGui.addFolder(oGuiElement, oOutputParameterHandler)
...
def multiPartMovie():
    oGui = cGui()
    oInputParameterHandler = cInputParameterHandler()
    sPartUrl = oInputParameterHandler.getValue('siteUrl')
    oHoster = oInputParameterHandler.getValue('Hoster')
    sTitle = oInputParameterHandler.getValue('sTitle')    
    cHosterGui().showHosterMenuDirect(oGui, oHoster, sPartUrl, sFileName=sTitle)
    oGui.setEndOfDirectory()
